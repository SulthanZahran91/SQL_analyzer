...
class SqlProcessRequest(BaseModel):
    ...
    include_subqueries: bool = Field(default=False, description="Whether to include subquery extraction")

class Component(BaseModel):
    type: str = Field(..., description="Component type (cte, table_variable_block, etc.)")
    name: str = Field(..., description="Component name")
    position: Optional[int] = Field(None, description="Position in SQL string")
...
class Statistics(BaseModel):
    ...
    breakdown: Dict[str, int] = Field(..., description="Breakdown by component type")
...
@app.post("/api/sql/process", response_model=SqlProcessResponse)
...
async def validate_sql(request: SqlProcessRequest):
    ...
    try:
        ...
        if len(clean_sql.strip()) < 5:
            return {"valid": False, "error": "SQL query appears to be too short"}
